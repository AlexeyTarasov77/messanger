...
export async function POST<T>(path: string | URL, data: any): APIResponse<T> {
...
export async function DELETE(path: string | URL): APIResponse<null> {
  return await sendReq(path, {
    method: "DELETE",
  });
}

export async function PATCH<T>(path: string | URL, data: any): APIResponse<T> {
  const headers: HeadersInit = {};
  if (!(data instanceof FormData)) {
    headers["Content-Type"] = "application/json";
    data = JSON.stringify(data);
