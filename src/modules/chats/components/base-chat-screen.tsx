...
interface BaseChatProps {
    ...
    setChat: React.Dispatch<
        React.SetStateAction<ChatGroupWithMessages | undefined>
    >;
    chatInfo: ReactNode;
...
export function BaseChatScreen({
    ...
    const [inputMessage, setInputMessage] = useState<MessageSendPayload | null>(
        null
    );
    ...
    const pickMessageImage = async () => {
        ...
        if (result && !result.canceled && result.assets) {
            ...
            setInputMessage((prev) =>
                prev
                    ? { ...prev, attached_image: imageUri }
                    : { attached_image: imageUri, content: "" }
            );
        }
    };

    useEffect(
        () => messagesBoxRef.current?.scrollToEnd({ animated: true }),
        [chat]
    );
    useEffect(() => {
        if (!socket) return;
    ...
    const menuButtons = [<MenuBtn Icon={ICONS.PostsIcon} label="Медіа" />];
    user.id === chat.admin_id
        ? menuButtons.push(
              <MenuBtn
                  Icon={ICONS.PenIcon}
