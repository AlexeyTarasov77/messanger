...
export function GroupCreateOrUpdateModal({
  ...
  const heading = { [ModalName.UPDATE_CHAT]: "Редагування групи", [ModalName.CREATE_CHAT_STEP_2]: "Нова група" }[name]
  const confirmBtnLabel = { [ModalName.UPDATE_CHAT]: "Зберегти зміни", [ModalName.CREATE_CHAT_STEP_2]: "Створити групу" }[name]
  return (
    <Modal isVisible={isVisible} onBackdropPress={close} coverScreen={false} className="bg-white justify-center items-center rounded-2xl my-auto" style={{ maxHeight: "70%" }}>
      <View className="px-10 pt-6 flex-1">
        <View className='gap-4'>
          ...
          <View className="gap-6 items-center justify-center">
            <Image source={{ uri: chatAvatar || DEFAULT_AVATAR_URL }} className="rounded-full w-12 h-12" />
          ...
          <View className={clsx("gap-4", name === ModalName.UPDATE_CHAT && "border border-border p-4 rounded-lg")}>
            ...
            <ScrollView className="max-h-52">
              {selectedMembers.map(contact => (
                <View key={contact.id} className="flex-row items-center gap-2 p-2">
                  <UserAvatar user={contact} className="w-12 h-12" />
        ...
        <View className="flex-row justify-end gap-2">
          <RoundedButton className="py-0" onPress={onCancel} label="Назад" />
          <RoundedButton className="py-0" onPress={() => onFormSubmit({ name: chatName, avatar: chatAvatar, selectedMembers })} label={confirmBtnLabel} filled darkFill />
        </View>
      </View>
    </Modal>
  );
