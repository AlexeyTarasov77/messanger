...
export function MembersSelectionModal({
    heading,
    confirmBtnLabel,
    onSubmit,
    close,
    ...
    onSubmit: (selectedMembers: IUser[]) => Promise<string | void>;
}) {
    ...
    const sortedContacts = Object.entries(groupedContacts).sort(
        ([letter1, _], [letter2, __]) => {
            if (letter1 < letter2) {
                return -1;
            }
            if (letter1 > letter2) {
    ...
    return (
        ...
        >
            <View className="pr-9 pl-9 p-6 flex-1 ">
                        ...
                        {sortedContacts.map(([letter, contacts]) => (
                            <View key={letter}>
                                ...
                                {contacts?.map((contact) => (
                                    <View
                                        key={contact.id}
                                        className="flex-row items-center gap-2 p-2 border-b border-gray-400"
                                    >
                                        <UserAvatar
                                            user={contact}
                ...
                <View className="flex-row justify-end gap-2 pt-10">
