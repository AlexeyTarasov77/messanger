...
export function Requests() {
  ...
  const removeRequest = async (fromUserId: number, action: "accept" | "decline") => {
    try {
      if (action === "accept") {
    ...
    setRequests(requests.filter(fromUser => Number(fromUser.id) !== fromUserId))
  };

  const acceptRequest = async (fromUserId: number) => await removeRequest(fromUserId, "accept");
  const declineRequest = async (fromUserId: number) => removeRequest(fromUserId, "decline");
