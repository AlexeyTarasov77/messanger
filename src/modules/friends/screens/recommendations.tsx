...
export function Recommendations({ showAll }: { showAll?: boolean }) {
  const { recommendations, isLoading, setRecommendations } = useRecommendations();
  const { user } = useUserCtx();
  useEffect(() => {
    const func = async () => {
      const excludedIdsString = await AsyncStorage.getItem(EXCLUDED_RECOMMENDED_USERS_KEY)
      if (!excludedIdsString) return
      const excludedIds = excludedIdsString.split(",")
      setRecommendations(recommendations.filter(user => !excludedIds.includes(String(user.id))))
    }
  ...
  const createFriendRequest = async (toUserId: number) => {
    try {
      ...
      setRecommendations(recommendations.filter(recommendedUser => recommendedUser.id !== toUserId))
    } catch (err) {
      console.error(err)
  ...
  const removeRecommendedUser = async (id: number) => {
    setRecommendations(recommendations.filter(recommendedUser => recommendedUser.id !== id))
